...
const backendPath = path.dirname(__filename)
...
const config: Configuration = {
  ...
  entry: {
    main: [
      path.resolve(backendPath, './src/main.ts')
    ],
  },
  output: {
  ...
  resolve: {
    ...
    alias: {
      "@src": path.resolve(backendPath, 'src'),
      '@config': path.resolve(backendPath, 'config'),
      '@entity': path.resolve(backendPath, 'src/entity')
    }
  },
  module: {
    rules: [
