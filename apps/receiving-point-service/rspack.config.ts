...
const config: Configuration = {
  ...
  output: {
    path: path.resolve(rootDir, 'dist/apps/receiving-point-service'),
    clean: true,
  },
  resolve: {
  ...
  plugins: [
    new CopyRspackPlugin({
      patterns: [
        {
          from: path.resolve(receivingPointServicePath, 'config'),
          to: path.resolve(rootDir, 'dist/apps/receiving-point-service/config'),
        },
      ],
    }),
    !process.env.BUILD &&
