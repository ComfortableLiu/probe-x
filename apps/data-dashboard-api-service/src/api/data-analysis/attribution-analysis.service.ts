...
export class AttributionAnalysisService {
  ...
  async createDownloadTask(data: ISubmitDownloadTaskReq, user: IUser) {
    const taskId = uuidv4()
    // 拼接SQL语句
    const { sql, params, error } = generateAttributionAnalysisSql(data as unknown as IAttributionAnalysisReq)
    ...
    // 记录任务
    await this.dataAnalysisRecordService.recordTask(taskId, '归因分析数据导出', user, JSON.stringify(data))
