...
export class UserPathAnalysisService {
  ...
  async createDownloadTask(data: ISubmitDownloadTaskReq, user: IUser) {
    const taskId = uuidv4()
    // 拼接SQL语句
    const { sql, params, error } = generateUserPathAnalysisSql(data as unknown as IUserPathAnalysisReq)
    ...
    // 记录任务
    await this.dataAnalysisRecordService.recordTask(taskId, '用户路径分析数据导出', user, JSON.stringify(data))
