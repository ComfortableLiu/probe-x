 ...
 */
@Entity('role', {
  comment: '角色表：存储系统中的角色信息，包括系统角色和用户自定义角色。系统角色（roleType=SYSTEM）由系统预定义，不可修改和删除；自定义角色（roleType=CUSTOM）由用户创建，可以修改和删除',
})
export class Role {
  /** 角色唯一ID（自增） */
  ...
  /** 角色显示名称（唯一，如 "管理员"、"普通用户"），用于前端页面展示 */
  @Column({
    type: 'varchar',
    length: 100,
    name: 'role_name',
    unique: true,
    comment: '角色显示名称（唯一，用于前端页面展示）',
  })
  roleName?: string
